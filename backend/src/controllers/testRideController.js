...
/* =====================================
   CREATE TEST RIDE (AUTO CONFIRM / REJECT)
===================================== */

export const createTestRide = async (req, res) => {
  try {
    const { bikeName, location, date, timeSlot, fullName, phone, email } =
      req.body;
    ...
    // 🔥 CHECK ONLY BIKE + DATE + TIME (NOT LOCATION)
    ...
    if (existingRide) {
      return res.status(400).json({
        message:
          "This bike is already booked for the selected date and time. Please choose another slot.",
      });
    }
...
export const getAllTestRides = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Failed to fetch test rides" });
  }
};
...
/* =====================================
   MARK AS VIEWED (ADMIN OPENS PAGE)
===================================== */
